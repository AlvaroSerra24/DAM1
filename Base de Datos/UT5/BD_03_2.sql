...
INSERT INTO emplea2 VALUES( 12222,  'MARIA',      'GALAN',    'RODRIGO',2050, 'C  MARIA ZAMBRANO,48',         'TALAVERA DE LA REINA', 'TOLEDO',    '45600', 'M', TO_DATE('15/10/2008'), TO_DATE('14/02/1957'),6); 
...
INSERT INTO emplea2 VALUES( 16666,  'MIGUEL',     'PEREZ',    'TORRICO',2050, 'AV DE LA CONSTITUCION,48',     'NAVALCARNERO',         'MADRID',    '28600', 'H', TO_DATE('21/01/2008'), TO_DATE('14/02/1990'),7); 
INSERT INTO emplea2 VALUES( 17777,  'PEDRO JOSE', 'CORTES',   'CRUZ',   1830, 'C  CASTELLANAS,3 1ºA',         'CADIZ',                'CADIZ',     '11000', 'H', TO_DATE('28/02/2010'), TO_DATE('10/09/1979'),8);
...
SELECT PROVINCIA, POBLACION, SEXO, COUNT(SALARIO) NUMERO
FROM EMPLEA2
WHERE DPTO_COD!=6 AND PROVINCIA LIKE 'TOLEDO'
GROUP BY PROVINCIA, POBLACION, SEXO
HAVING SEXO LIKE 'H'
ORDER BY NUMERO;

/*EJERCICIO 1*/

SELECT LPAD(DPTO_COD,3,0)"CODIGO", NOMBRE_DPTO"DEPARTAMENTO", TO_CHAR(PRESUPUESTO, '999G999L')"PRESUPUESTO"
FROM DEPARTAMENTOS
WHERE PRESUPUESTO<(SELECT PRESUPUESTO
FROM DEPARTAMENTOS
WHERE NOMBRE_DPTO LIKE 'SISTEMAS');

/*EJERCICO 3*/

SELECT NOMBRE|| ' ,' || APELLIDO1||' '||APELLIDO2 "NOMBRE COMPLETO", TRUNC(MONTHS_BETWEEN(SYSDATE,FECHA_NAC)/12) "EDAD", FECHA_NAC,TO_CHAR(FECHA_NAC,'MONTH') "MES DE NACIMIENTO", TO_CHAR(ROUND(SALARIO,2),'999G999L')"SALARIO"
FROM EMPLEA2
...
SELECT NOMBRE|| ' ,' || APELLIDO1||' '||APELLIDO2 "NOMBRE COMPLETO", DIRECC, TRUNC(MONTHS_BETWEEN(SYSDATE,FECHA_NAC)/12) "EDAD", FECHA_NAC,TO_CHAR(FECHA_NAC,'MONTH') "MES DE NACIMIENTO", TO_CHAR(ROUND(SALARIO,2),'999G999L')"SALARIO"
FROM EMPLEA2
...
SELECT LPAD(DPTO_COD,3,'0')"CODIGO", INITCAP(NOMBRE_DPTO)"DEPARTAMENTO", TO_CHAR(PRESUPUESTO,'999G999L')"PRESUPUESTO"
FROM DEPARTAMENTOS
WHERE EXISTS (SELECT DPTO_COD
FROM EMPLEA2
WHERE TRUNC(MONTHS_BETWEEN(SYSDATE,FECHA_NAC)/12)>25);

/*EJERCICIO 6*/

SELECT INITCAP(NOMBRE || ' ' || APELLIDO1)"EMPLEADO", TO_CHAR(TRUNC(MONTHS_BETWEEN(SYSDATE,FECHA_INICIO)/12),'99')|| 'AÑOS' "ANTIGUEDAD", TO_CHAR(SALARIO, '999G999L')"SALARIO", DPTO_COD "DEPARTAMENTO"
FROM EMPLEA2
WHERE SALARIO BETWEEN (SELECT AVG(SALARIO)
FROM EMPLEA2
WHERE DPTO_COD=(SELECT DPTO_COD
FROM DEPARTAMENTOS
WHERE TRIM(NOMBRE_DPTO) LIKE 'SEGURIDAD'))
AND (SELECT AVG (SALARIO)
FROM EMPLEA2
WHERE DPTO_COD=(SELECT DPTO_COD
FROM DEPARTAMENTOS
WHERE TRIM (NOMBRE_DPTO) LIKE 'HARDWARE'));

/*EJERCICIO 7*/

SELECT NOMBRE, TO_CHAR (SALARIO, '9G999D99L')"SALARIO"
FROM EMPLEA2
WHERE SALARIO>(SELECT MAX(SLARIO)
FROM EMLEA2
WHERE DPTO_COD=(SELECT DPTO_COD
FROM DEPARTAMENTOS
WHERE))
...
SELECT INITCAP(NOMBRE_DPTO)"DEPARTAMENTO"
FROM DEPARTAMENTOS
WHERE PRESUPUESTO=(SELECT MIN(PRESUPUESTO)
FROM DEPARTAMENTOS)
AND DNI_JEFE_DPTO IN (SELECT DNI
FROM EMPLEA2
WHERE PROVINCIA LIKE 'TOLEDO');

/*EJERCICIO 9*/

SELECT NOMBRE_DPTO, TO_CHAR(PRESUPUESTO, '999G999L')"PRESUPUESTOS", DNI_JEFE_DPTO
FROM DEPARTAMENTOS
...
SELECT NOMBRE_DPTO, TO_CHAR(PRESUPUESTO, '999G999L')"PRESUPUESTOS"
FROM DEPARTAMENTOS
WHERE PRESUPUESTO > 5000
AND EXISTS (SELECT SALARIO
FROM EMPLEA2
WHERE SALARIO > (SELECT MAX (SALARIO)
FROM EMPLEA2
WHERE DPTO_COD=(SELECT DPTO_COD
FROM DEPARTAMENTOS
WHERE TRIM(NOMBRE_DPTO) LIKE 'SISTEMAS')));

/*EJERCICIO 12*/

SELECT NOMBRE  ||'-'||APELLIDO1||' '||APELLIDO2 "NOMBRE COMPLETO", TO_CHAR(SALARIO, '9999D99')"SALARIO",
TRUNC (MONTHS_BETWEEN (SYSDATE,FECHA_NAC)/12)"EDAD",
FLOOR (SYSDATE-FECHA_INICIO)"ANTIGUEDAD EN DIAS"
FROM EMPLEA2
WHERE INSTR (NOMBRE, ' ' )!=0
AND LENGTH (NOMBRE)>=10
AND (MONTHS_BETWEEN(SYSDATE, FECHA_NAC)/12)<(SELECT AVG(MONTHS_BETWEEN(SYSDATE,FECHA_NAC)/12)
FROM EMPLEA2)
AND (MONTHS_BETWEEN(SYSDATE, FECHA_INICIO)/12)<(SELECT AVG(MONTHS_BETWEEN(SYSDATE,FECHA_INICIO)/12)
FROM EMPLEA2);