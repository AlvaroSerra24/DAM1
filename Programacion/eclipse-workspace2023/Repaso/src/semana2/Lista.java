...
public class Lista {
	public static void main(String[] args) {
		...
		System.out.println("Imprimir Streams() II");
		list.forEach(System.out::println);
		
		//Contar los numero pares de  la lista
		int contador=0;
		for (Integer i: list) {
			if(i%2==0) {
				contador++;
		...
		//Crear una lista nueva con los numeros pares
		...
		nueva=list.stream().filter(n->n%2==0).toList();//.collect(Collectors.toList());
		...
		if(menor10)
			System.out.println("Hay números menores que 10");
		else
			System.out.println("No hay números menores que 10");
		
		if(list.stream().anyMatch(n->n<10))
			System.out.println("Hay números menores que 10");
		else
